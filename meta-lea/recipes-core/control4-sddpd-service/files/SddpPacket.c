...
static int parseHeaderLine(SddpPacket *packet, const char *line, const char *lineEnd)
{
    ...
    {
        if (packet->driver == NULL)
            packet->driver = dupValue(value, lineEnd);
    }
    else if (isHeader(header, headerLen, "Config-URL"))
    {
        if (packet->config_url == NULL)
            packet->config_url = dupValue(value, lineEnd);
    }
...
void FreeSDDPPacket(SddpPacket *packet)
{
	...
	freePacketHeader(&packet->config_url);
...
size_t WriteSDDPPacket(const SddpPacket *packet, char *buffer, size_t bufferSize)
{
	...
	writeHeader(&written, buffer, bufferSize, "Config-URL", packet->config_url, 1);
