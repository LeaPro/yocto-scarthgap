...
typedef struct
{
    ...
    char *config_url;    // The url of the device configuration page
    int  max_age;        // Number of seconds advertisement is valid (e.g. 1800)
...
// Private Functions
///////////////////////////////////////
static SDDPStatus SDDPNotify(SDDPState *sddp_state, SDDPDevice *sddp_dev, SDDPMessageType type, struct sockaddr_in *dest, struct in_addr *iface_addr);
...
SDDPStatus SDDPGetType(const SDDPHandle handle, const char * type)
{
    ...
    if (sddp_state == NULL)
    {
        ...
        syslog(LOG_INFO, "SDDPGetType failed");
        return SDDP_STATUS_INVALID_PARAM;
    }
    SDDPDeviceConfig *config = &(sddp_state->devices[0].config);
    strcpy(type,config->product_name);
    return SDDP_STATUS_SUCCESS;
}
...
/******************************************************************************
 @fn        SDDPStatus SDDPSetDevice(const SDDPHandle handle, int index, const char *product_name,
                                     ...
                                     const char *driver, const char *config_url,
                                     int max_age, bool local_only)
 ...
 @param     handle - handle to SDDP
    		...
    		config_url - The url of the device configuration page
						 ...
						 int max_age, bool local_only)
{
	...
	if (dev->config.config_url)
    {
		free(dev->config.driver);
    }
	dev->config.config_url = dupStr(config_url);
...
SDDPStatus SDDPStart(const SDDPHandle handle)
{
    ...
    for (dev = sddp_state->devices; dev; dev = dev->next)
    {
        ...
        // Send startup notify
        status = SDDPNotify(sddp_state, dev, SDDP_MULTICAST, NULL, NULL);
...
/******************************************************************************
 @fn        SDDPStatus SDDPTick(const SDDPHandle handle, int forceAlive)
 ...
 @param     handle - handle to SDDP
            forceAlive - If nonzero, forces a alive messages to be broadcast
...
*******************************************************************************/
SDDPStatus SDDPTick(const SDDPHandle handle, int forceAlive)
{
	SDDPState *sddp_state = (SDDPState *)handle;
    ...
    for (dev = sddp_state->devices; dev; dev = dev->next)
    {
        if(dev->config.max_age != DISABLE_PERIODIC_ADVERTS &&
           (difftime(time(NULL), dev->private_state.time_prev) > dev->private_state.alive_interval || forceAlive))
        {
            if (forceAlive)
                syslog(LOG_INFO, "Sending SDDP advertisement");
            else
                syslog(LOG_INFO, "Sending Periodic SDDP advertisement");

            time(&dev->private_state.time_prev);
            dev->private_state.search_response = false;
            if(SDDPNotify(sddp_state, dev, SDDP_MULTICAST, NULL, NULL) != SDDP_STATUS_SUCCESS)
            {
    ...
    recv_len = SDDPNetworkReceive(&sddp_state->network_info, data, sizeof(data), (struct sockaddr*)&from, &from_len, &iface_addr);
    if (recv_len <= 0)
    {
    			...
    			if (!strcmp(received->h.request.method, "SEARCH"))
    			{
    				for (dev = sddp_state->devices; dev; dev = dev->next)
    				{
	    				     ...
	    				     dev->config.product_name[strlen(received->h.request.argument)] == ':'))
	    				{
	    					...
	    					status = SDDPNotify(sddp_state, dev, SDDP_UNICAST, &from, &iface_addr);
...
/******************************************************************************
 @fn        static SDDPStatus SDDPNotify(SDDPState *sddp_state, SDDPDevice *sddp_dev,
                                                    SDDPMessageType type, struct sockaddr_in *dest, struct in_addr *iface_addr)
 ...
 @param     iface_addr - the interface to respond to, for unicast
...
static SDDPStatus SDDPNotify(SDDPState *sddp_state, SDDPDevice *sddp_dev, SDDPMessageType type, struct sockaddr_in *dest, struct in_addr *iface_addr)
{
    ...
    // Check parameters
    if (type == SDDP_UNICAST && (dest == NULL || iface_addr == NULL))
    {
    ...
    for (ifa = iface; ifa != NULL; ifa = ifa->next)
    {
        ...
        {
            // only send in the interface that received the request
            if (iface_addr->s_addr == ifa->addr.s_addr)
            {
                // Send unicast
...
static SDDPStatus SDDPCreateNotify(SDDPState *sddp_state, SDDPDevice *sddp_dev, char *dst, int max_len, const SDDPInterface *iface)
{
	static char version[] = SDDP_VERSION;
	...
	if (devInfo)
	{
		...
		if (sddp_dev->config.config_url[0] != '\0')
		{
			tmpPtr = calloc(1, strlen(sddp_dev->config.config_url) + strlen(iface->ip_addr));
			char *copy = strdup(sddp_dev->config.config_url);
			char *token = strtok(copy, "<>");
			while (token != NULL)
			{
				if (strcmp(token, "host") == 0)
					strcat(tmpPtr, iface->ip_addr);
				else
					strcat(tmpPtr, token);
				token = strtok(NULL, "<>");
			}
...
SDDPStatus SDDPIsConfigValid(const SDDPDeviceConfig *sddp_config)
{
    // Check for NULL pointers (except for config_url is is optional)
