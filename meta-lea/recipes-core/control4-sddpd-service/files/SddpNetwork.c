...
SDDPStatus SDDPNetworkInit(SDDPNetworkInfo *network)
{
    ...
    {
        syslog(LOG_INFO, "Can't set SO_REUSEADDR on multicast listener socket: %s", strerror(errno));
    }

    // Enable packet info, gives us the interface it came in on
    socket_opt = 1;
    if (setsockopt(network->listener_socket, SOL_IP, IP_PKTINFO, &socket_opt, sizeof(int)))
    {
        syslog(LOG_INFO, "Can't set IP_PKTINFO on multicast listener socket: %s", strerror(errno));
    }
...
SDDPStatus SDDPGetUniqueHostName(void *handle, char *uniqueHostName, int maxLen)
{
  int fd;
  struct ifreq ifr;
  uint8_t mac[6];
  ...
  // combine device type and MAC to make a unique hostname, e.g. leaamp-34d-9006F2C8F2E0
  // 1) get type, e.g. lea:amp:34d
  // 2) copy characters from type to uniqueHostname, skipping the first ':' and replacing the second ':' with '-'
  // 3) append -<MAC>

  // step 1
  SDDPGetType(handle, typeString);

  // step 2
  char *p = typeString, *q = uniqueHostName;
  while (*p)
  {
    if (*p != ':')
    {
  ...
  *q = '\0';
  ...
  strncpy(ifr.ifr_name, "eth0", IFNAMSIZ - 1); // IP address attached to "interface"
  
  struct sockaddr_in *s = (struct sockaddr_in *)&ifr.ifr_addr; // addresses get returned in a struct sockaddr_in
  ...
  strcat(uniqueHostName, macString);
...
/******************************************************************************
 ...
 @param     iface_addr - the address of the interface the message came in on
                       ...
                       struct in_addr *iface_addr)
{
    ...
    result = recvmsg(network->listener_socket, &message, 0);

    if (result > 0)
    {
        for(messageHeader = CMSG_FIRSTHDR(&message); messageHeader != NULL; messageHeader = CMSG_NXTHDR(&message, messageHeader))
        {
            if (messageHeader->cmsg_level == IPPROTO_IP && messageHeader->cmsg_type == IP_PKTINFO)
                *iface_addr = ((struct in_pktinfo*) CMSG_DATA(messageHeader))->ipi_spec_dst;
        }
    }
