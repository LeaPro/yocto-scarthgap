...
/******************************************************************************
 @fn        SDDPStatus SDDPGetUniqueHostName(void *handle, char *uniqueHostName, int maxLen)

 @brief     implementation-specific; for LEA amps, it currently uses hostname + eth0 MAC


 @return    SDDP_STATUS_SUCCESS if ok
            SDDP_STATUS_NETWORK_ERROR on error
...
*******************************************************************************/
int SDDPNetworkReceive(SDDPNetworkInfo *network, char *buf, int buf_len, struct sockaddr *from, unsigned int *from_len, struct in_addr *iface_addr);
