SUMMARY = "SuperDARC application server"
DESCRIPTION = "Service that runs the SuperDARC application server"
LICENSE = "CLOSED"
...
SRC_URI =  " \
    ...
    file://ipcTool \
    file://kvsTool \
    file://PSBS.zip;unpack=0 \
    file://PSOK.zip;unpack=0 \
    file://AMBS.zip;unpack=0 \
...
do_install () {
    ...
    install -m 0755 ${WORKDIR}/PSBS.zip ${D}/${sbindir}
    install -m 0755 ${WORKDIR}/PSOK.zip ${D}/${sbindir}
    install -m 0755 ${WORKDIR}/AMBS.zip ${D}/${sbindir}
