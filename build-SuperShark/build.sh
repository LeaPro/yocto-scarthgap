...
INFILE=./tmp/deploy/images/lea-supershark/core-image-minimal-lea-supershark.rootfs.tar.xz
OUTFILE=postBuild/images/SS-${VERSION}-LEA-SuperShark-${DATE}.tar.xz.enc
mkdir -p postBuild/images
rm -rf $OUTFILE
openssl enc -aes-256-cbc -salt -in $INFILE -out $OUTFILE -k SSSS
